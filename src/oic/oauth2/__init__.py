...
class Client(PBase):
    ...
    def construct_request(self, request: Type[Message], request_args=None, extra_args=None):
        # FIXME: Annotating the return value as `Message` disrupts
        # all the methods that are calling this :(
    ...
    def get_urlinfo(self, info: str) -> str:
    ...
    def do_authorization_request(self, request=AuthorizationRequest,
                                 ...
                                 **kwargs):
        # FIXME: Cannot annotate the return value since the message class doesn't have
        # the methods and attributes on itself but in `_dict`
        if state:
            try:
